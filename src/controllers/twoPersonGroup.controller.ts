...
import { z } from "zod";
import twoPersonGroupService from "../services/twoPersonGroup.service";
...
export const registerTwoPersonGroupSchema = z.object({
  body: z.object({
    members: z
      .array(memberSchema)
      .length(2, "Exactly 2 members are required for a two-person group package"),
    payerEmail: z.string().email(),
  }),
});

export const initializeTwoPersonGroupPaymentSchema = z.object({
  params: z.object({
...
export const getTwoPersonGroupSchema = z.object({
  params: z.object({
    groupId: z.string().min(1),
  }),
});

export class TwoPersonGroupController {
  async registerGroup(
    req: Request,
    res: Response,
    next: NextFunction,
    ...
    try {
      ...
      const result = await twoPersonGroupService.registerGroup(
        members,
        payerEmail,
      );
      ...
      res.status(200).json({
        success: true,
        message:
          "Two-person group registered successfully. Use the groupId to initialize payment.",
        data: result,
      });
    } catch (error) {
      next(error);
  ...
  async initializePayment(
    ...
    try {
      ...
      const result = await twoPersonGroupService.initializeGroupPayment(
        groupId,
        amount,
        payerEmail,
      );
      ...
      res.status(200).json({
        success: true,
        message: "Two-person group payment initialized successfully",
        data: result,
      });
    } catch (error) {
      next(error);
  ...
  async getGroup(
    ...
    try {
      const { groupId } = req.params;
      const group = await twoPersonGroupService.getGroupById(groupId);
...
export default new TwoPersonGroupController();
